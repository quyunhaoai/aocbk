...
- (void)viewDidLoad {
    ...
    self.loginInputView.regestLabel.delegate = self;
    ...
    [self.loginInputView.regestLabel addTextStorage:linkStorage];
}
...
- (void)jumpBtnClicked:(id)item {
    if ([item isKindOfClass:[UIButton class]]) {
        ...
        if (tag == BUTTON_TAG(1)) {
            //手机号登录
            STMobileLoginViewController *vc = [STMobileLoginViewController new];
