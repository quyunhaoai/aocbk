...
- (instancetype)initWithTitle:(NSString *)title
    ...
    if(self){
        ...
        _contentOffsetY = STATUS_BAR_HEIGHT / 2.0 ;
        [self setupUI];
    }
    return self;
}
...
- (void)setupUI{
    ...
    for(UIButton *btn in self.leftBtns){
        ...
        [btn mas_updateConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(btn.frame.size);
    ...
    [self.titleLabel mas_updateConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self).mas_offset(2 * 10);
        make.centerY.mas_equalTo(self).mas_offset(self.contentOffsetY);
        make.width.mas_equalTo(self).mas_offset(-4 * 10).priority(998);
        make.height.mas_equalTo(20);
    }];
    
    lastBtn = nil ;
    for(NSInteger i = self.rightBtns.count - 1 ; i >=0 ; i--){
        UIButton *btn = [self.rightBtns objectAtIndex:i];
        [self addSubview:btn];
        [btn mas_updateConstraints:^(MASConstraintMaker *make) {
           make.size.mas_equalTo(btn.frame.size); make.centerY.mas_equalTo(self).mas_offset(self.contentOffsetY);
            if(lastBtn == nil){
...
- (void)setRightBtns:(NSArray *)rightBtns{
    ...
    for(NSInteger i = _rightBtns.count - 1 ; i >=0 ; i--){
        UIButton *btn = [_rightBtns objectAtIndex:i];
        [self addSubview:btn];
        [btn mas_updateConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(btn.frame.size);
            make.centerY.mas_equalTo(self).mas_offset(self.contentOffsetY);
...
- (void)setTitleView:(UIView *)titleView{
    ...
    [_titleView mas_updateConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.mas_equalTo(self).mas_offset(self.contentOffsetY);
        make.left.mas_equalTo(self).mas_offset(titleView.frame.origin.x);
    }];
}
