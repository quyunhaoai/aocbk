...
- (UIView *)backGreenView
{
    if (!_backGreenView) {
        ...
        _backGreenView.backgroundColor = kMainColor;
    }
...
{
    if (!_playTimeLabel) {
        _playTimeLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, (self.tempFrame.size.height - 15)/2.0f, self.playTimeW, 15)];
        _playTimeLabel.font = FONT_10;
        ...
        _playTimeLabel.backgroundColor = [UIColor colorWithRed:65/255.0 green:65/255.0 blue:65/255.0 alpha:1.0];
        _playTimeLabel.layer.masksToBounds = YES;
        _playTimeLabel.layer.cornerRadius = 15 / 2.0f;
...
-(void)doMoveAction:(UIPanGestureRecognizer *)recognizer{
    ...
    newCenter.y = MIN(self.frame.size.height - recognizer.view.frame.size.height/2-(self.tempFrame.size.height - 15)/2.0f, recognizer.view.frame.size.width/2);
