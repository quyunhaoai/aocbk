    ...
    switch (eventType) {
        case AVPEventPrepareDone: {
            ...
            if (self.playMethod == ALYPVPlayMethodUrl) {
                ...
                [self updateControlLayerDataWithMediaInfo:[player getMediaInfo]];
                AVPMediaInfo *info = [self getAliyunMediaInfo];
                NSLog(@"getAliyunMediaInfo:%@",info.mediaType);
            }
        }
            break;
...
- (void)onCurrentPositionUpdate:(AliPlayer*)player position:(int64_t)position {
    ...
    if(_mProgressCanUpdate == YES){
        ...
        if (self.isAudio) {
            if (self.delegate && [self.delegate respondsToSelector:@selector(updateProgressWithCurrentTime:durationTime:)]) {
                [self.delegate updateProgressWithCurrentTime:currentTime durationTime:durationTime];
            }
        } else {
...
- (void)onPlayerStatusChanged:(AliPlayer*)player oldStatus:(AVPStatus)oldStatus newStatus:(AVPStatus)newStatus {
    ...
    //更新UI状态
    if (self.isAudio) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(updateViewWithPlayerState:)]) {
            [self.delegate updateViewWithPlayerState:_currentPlayStatus];
        }
    } else {
...
- (void)updateControlLayerDataWithMediaInfo:(AVPMediaInfo *)mediaInfo {
    if ([mediaInfo.mediaType isEqualToString:@"audio"]) {
        //音频
        self.isAudio = YES;
        [self.popLayer removeFromSuperview];
        return;
    } else {
        self.isAudio = NO;
    }
