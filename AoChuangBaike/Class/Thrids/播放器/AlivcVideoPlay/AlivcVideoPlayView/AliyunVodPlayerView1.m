...
@interface AliyunVodPlayerView1 () <AVPDelegate,AliyunPVPopLayerDelegate,AliyunControlViewDelegate,AliyunViewMoreViewDelegate>
...
@implementation AliyunVodPlayerView1
...
- (void)handleDeviceOrientationDidChange:(UIInterfaceOrientation)interfaceOrientation {
    ...
    switch (device.orientation) {
        ...
        case UIDeviceOrientationLandscapeRight: {
            ...
            if (self.delegate &&[self.delegate respondsToSelector:@selector(AliyunVodPlayerView1:fullScreen:)]) {
                [self.delegate AliyunVodPlayerView1:self fullScreen:YES];
            }
        }
            break;
        case UIDeviceOrientationPortrait: {
            ...
            if (self.delegate &&[self.delegate respondsToSelector:@selector(AliyunVodPlayerView1:fullScreen:)]) {
                [self.delegate AliyunVodPlayerView1:self fullScreen:isFullScreen];
            }
...
-(void)onPlayerEvent:(AliPlayer*)player eventType:(AVPEventType)eventType {
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(AliyunVodPlayerView1:happen:)]){
        [self.delegate AliyunVodPlayerView1:self happen:eventType];
    }
    ...
    switch (eventType) {
        ...
        case AVPEventCompletion: {
            if (self.delegate && [self.delegate respondsToSelector:@selector(onFinishWithAliyunVodPlayerView1:)]) {
                [self.delegate onFinishWithAliyunVodPlayerView1:self];
            }
            [self unlockScreen];
        }
            break;
...
- (void)showPopViewWithType:(ALYPVErrorType)type {
    ...
    switch (type) {
        case ALYPVErrorTypeReplay: {
            //重播
            [self.aliPlayer seekToTime:self.saveCurrentTime*1000 seekMode:self.seekMode];
            [self start];
        }
            break;
        ...
        case ALYPVErrorTypeStsExpired: {
            if ([self.delegate respondsToSelector:@selector(onSecurityTokenExpiredWithAliyunVodPlayerView1:)]) {
                [self.delegate onSecurityTokenExpiredWithAliyunVodPlayerView1:self];
            }
        }
            break;
        default:
...
- (void)onBackClickedWithAlPVPopLayer:(AliyunPlayerViewPopLayer *)popLayer {
    if(self.delegate && [self.delegate respondsToSelector:@selector(onBackViewClickWithAliyunVodPlayerView1:)]){
        [self.delegate onBackViewClickWithAliyunVodPlayerView1:self];
    }else{
        [self stop];
    }
}
...
- (void)unlockScreen{
    //弹出错误窗口时 取消锁屏。
    if (self.delegate &&[self.delegate respondsToSelector:@selector(AliyunVodPlayerView1:lockScreen:)]) {
        if (self.isScreenLocked == YES||self.fixedPortrait) {
            [self.delegate AliyunVodPlayerView1:self lockScreen:NO];
            //弹出错误窗口时 取消锁屏。
...
- (void)onDownloadButtonClickWithAliyunControlView:(AliyunPlayerViewControlView *)controlViewP {
    if (self.delegate && [self.delegate respondsToSelector:@selector(onDownloadButtonClickWithAliyunVodPlayerView1:)]) {
        [self.delegate onDownloadButtonClickWithAliyunVodPlayerView1:self];
    }
}
...
- (void)onClickedfullScreenButtonWithAliyunControlView:(AliyunPlayerViewControlView *)controlView {
    if(self.fixedPortrait){
        ...
        if (self.delegate &&[self.delegate respondsToSelector:@selector(AliyunVodPlayerView1:fullScreen:)]) {
            [self.delegate AliyunVodPlayerView1:self fullScreen:self.isProtrait];
        }
    }else{
        if(self.isScreenLocked){
            return;
...
- (void)onLockButtonClickedWithAliyunControlView:(AliyunPlayerViewControlView *)controlView{
    ...
    if (self.delegate &&[self.delegate respondsToSelector:@selector(AliyunVodPlayerView1:lockScreen:)]) {
        ...
        [self.delegate AliyunVodPlayerView1:self lockScreen:lScreen];
    }
}
