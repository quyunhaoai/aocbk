//
//  AliyunVodPlayerView1.h
//  AliyunVodPlayerView1SDK
//
 ...
 */
@class AliyunVodPlayerView1;

@protocol AliyunVodPlayerView1Delegate <NSObject>

//界面
/**
 * 功能：返回按钮事件
 * 参数：playerView ：AliyunVodPlayerView1
 */
- (void)onBackViewClickWithAliyunVodPlayerView1:(AliyunVodPlayerView1*)playerView;

/**
 * 功能：下载按钮事件
 * 参数：playerView ：AliyunVodPlayerView1
 */
- (void)onDownloadButtonClickWithAliyunVodPlayerView1:(AliyunVodPlayerView1*)playerView;

/**
 * 功能：所有事件发生的汇总
 * 参数：event ： 发生的事件
 */
- (void)AliyunVodPlayerView1:(AliyunVodPlayerView1*)playerView happen:(AVPEventType )event;

/**
 * 功能：暂停事件
 * 参数：currentPlayTime ： 暂停时播放时间
 */
- (void)AliyunVodPlayerView1:(AliyunVodPlayerView1*)playerView onPause:(NSTimeInterval)currentPlayTime;

/**
 * 功能：继续事件
 * 参数：currentPlayTime ： 继续播放时播放时间。
 */
- (void)AliyunVodPlayerView1:(AliyunVodPlayerView1*)playerView onResume:(NSTimeInterval)currentPlayTime;

/**
 * 功能：播放完成事件 ，请区别stop（停止播放）
 * 参数：playerView ： AliyunVodPlayerView1
 */
- (void)onFinishWithAliyunVodPlayerView1:(AliyunVodPlayerView1*)playerView;

/**
 * 功能：停止播放
 * 参数：currentPlayTime ： 播放停止时播放时间。
 */
- (void)AliyunVodPlayerView1:(AliyunVodPlayerView1*)playerView onStop:(NSTimeInterval)currentPlayTime;

/**
 * 功能：拖动进度条结束事件
 * 参数：seekDoneTime ： seekDone时播放时间。
 */
- (void)AliyunVodPlayerView1:(AliyunVodPlayerView1*)playerView onSeekDone:(NSTimeInterval)seekDoneTime;

/**
 * 功能：是否锁屏
 */
- (void)AliyunVodPlayerView1:(AliyunVodPlayerView1*)playerView lockScreen:(BOOL)isLockScreen;

/**
 * 功能：返回调用全屏
 * 参数：isFullScreen ： 点击全屏按钮后，返回当前是否全屏状态
 */
- (void)AliyunVodPlayerView1:(AliyunVodPlayerView1 *)playerView fullScreen:(BOOL)isFullScreen;

/**
 * 功能：循环播放开始
 * 参数：playerView ：AliyunVodPlayerView1
 */
- (void)onCircleStartWithVodPlayerView:(AliyunVodPlayerView1 *)playerView;

/**
 sts token过期回调

 @param playerView AliyunVodPlayerView1
 */
- (void)onSecurityTokenExpiredWithAliyunVodPlayerView1:(AliyunVodPlayerView1 *)playerView;

- (void)onClickedAirPlayButtonWithVodPlayerView:(AliyunVodPlayerView1 *)playerView;

- (void)onClickedBarrageBtnWithVodPlayerView:(AliyunVodPlayerView1 *)playerView;

@end

/*
 AliyunVodPlayerView1为带皮肤界面的播放器，可以直接作为View来使用
 */
@interface AliyunVodPlayerView1 : UIView
...
/*
 功能：设置AliyunVodPlayerView1Delegate代理
 */
@property (nonatomic, weak) id<AliyunVodPlayerView1Delegate> delegate;
...
- (void)AliyunVodPlayerView1:(AliyunVodPlayerView1 *)playerView lockScreen:(BOOL)isLockScreen此方法在isLockPortrait==yes时，返回的islockscreen总是yes；
...
/**
 ...
 参数：
    frame:视图view大小
    skin: 皮肤样式，类型有：
        AliyunVodPlayerView1SkinBlue,
        AliyunVodPlayerView1SkinRed,
        AliyunVodPlayerView1SkinOrange,
        AliyunVodPlayerView1SkinGreen
 */
