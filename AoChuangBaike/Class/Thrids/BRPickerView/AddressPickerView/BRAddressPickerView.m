...
- (void)loadData {
    ...
    //
    //        NSString *filePath = [plistBundle pathForResource:@"Area" ofType:@"plist"];
    ...
    NSMutableArray *array = dict[@"countries"];
    
    NSDictionary *pron = array.firstObject;
    NSArray *arr = pron[@"provinces"];
    NSLog(@"3333\n\n\n%@\n\n",arr);
...
- (void)parseDataSource {
    ...
    for (NSDictionary *proviceDic in self.dataSource) {
        ...
        proviceModel.provinceName = proviceDic[@"provinceName"];
        proviceModel.provinceID = proviceDic[@"provinceID"];
        NSArray *citylist = proviceDic[@"cities"];
        
        NSMutableArray *tempArr2 = [NSMutableArray array];
        for (NSDictionary *cityDic in citylist) {
            BRCityModel *cityModel = [[BRCityModel alloc]init];
            cityModel.cityID = cityDic[@"cityID"];
            cityModel.cityName = cityDic[@"cityName"];
            NSArray *arealist = cityDic[@"regions"];
            ...
            for (NSDictionary *areaDic in arealist) {
                ...
                areaModel.regionID = areaDic[@"regionID"];
                areaModel.regionName = areaDic[@"regionName"];
                [tempArr3 addObject:areaModel];
            }
            cityModel.regions = [tempArr3 copy];
            [tempArr2 addObject:cityModel];
        }
        proviceModel.cities = [tempArr2 copy];
        [tempArr1 addObject:proviceModel];
    }
    self.provinceModelArr = [tempArr1 copy];
    
    NSLog(@"%@++++++++%@+++++++++%@",self.provinceModelArr,self.cityModelArr,self.areaModelArr);
}
...
- (void)setupDefaultValue {
    ...
    [self.provinceModelArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([model.provinceName isEqualToString:selectProvinceName]) {
    ...
    if (self.showType == BRAddressPickerModeCity || self.showType == BRAddressPickerModeArea) {
        ...
        [self.cityModelArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if ([model.cityName isEqualToString:selectCityName]) {
                self->_cityIndex = idx;
    ...
    if (self.showType == BRAddressPickerModeArea) {
        ...
        [self.areaModelArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if ([model.regionName isEqualToString:selectAreaName]) {
                self->_areaIndex = idx;
...
- (NSArray *)getCityModelArray:(NSInteger)provinceIndex {
    ...
    // 返回城市模型数组
    return provinceModel.cities;
}

// 根据 省索引和城市索引 获取 区域模型数组
- (NSArray *)getAreaModelArray:(NSInteger)provinceIndex cityIndex:(NSInteger)cityIndex {
    BRProvinceModel *provinceModel = self.provinceModelArr[provinceIndex];
    BRCityModel *cityModel = provinceModel.cities[cityIndex];
    // 返回地区模型数组
    return cityModel.regions;
}
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    UIView *bgView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, (self.alertView.frame.size.width) / 3, 50 * kScaleFit)];
    bgView.backgroundColor = [UIColor clearColor];
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(5 * kScaleFit, 0, (self.alertView.frame.size.width) / 3 - 10 * kScaleFit, 50 * kScaleFit)];
    ...
    if (component == 0) {
        ...
        if (_selectProvinceModel != nil) {
            if ([model isEqual:_selectProvinceModel]) {
        ...
        if (_selectCityModel != nil) {
            if ([model isEqual:_selectCityModel]) {
                label.textColor = [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1.0];
                label.font = [UIFont fontWithName:@"PingFangSC-Medium" size: 18];
            } else {
                label.textColor = [UIColor colorWithRed:153/255.0 green:153/255.0 blue:153/255.0 alpha:1.0];
        ...
        if (_selectAreaModel != nil) {
            if ([model isEqual:_selectAreaModel]) {
                label.textColor = [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1.0];
                label.font = [UIFont fontWithName:@"PingFangSC-Medium" size: 18];
            } else {
                label.textColor = [UIColor colorWithRed:153/255.0 green:153/255.0 blue:153/255.0 alpha:1.0];
...
- (BRCityModel *)selectCityModel {
    if (!_selectCityModel) {
        ...
        _selectCityModel.cityID = @"";
        _selectCityModel.cityName = @"";
    }
...
- (BRAreaModel *)selectAreaModel {
    if (!_selectAreaModel) {
        ...
        _selectAreaModel.regionName = @"";
        _selectAreaModel.regionID = @"";
    }
